...
- Historical bootstrap evidence: `npm run synth` synthesized the original single stack without credentials (deprecation warnings from within aws-cdk-lib were expected and harmless). The current two-stack app additionally requires representative `domainName` and `certificateArn` contexts as shown above.
