...
| Integration test      | `tests/integration/end-to-end.test.ts`                | The critical journey over real HTTP against the dev server (same pipeline composition as production).                                                     |
| Smoke check           | `npm run smoke`                                       | Deterministic end-to-end run suitable for a fresh checkout: register → upload → complete → deploy → status → history → 401.                               |

## Required Before Merge

`npm run check` (format, lint, typecheck, tests, build, smoke, architecture rules, docs checks) plus `npm run synth`. CI (`.github/workflows/ci.yml`) runs exactly these.
